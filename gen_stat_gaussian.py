...
from models.reverse_gaussian import ReverseGaussian
...
for attack_type in [L1PGDAttack, L2PGDAttack]:
    ...
    for adv_train in adv_trains:
        ...
        stats[attack_type.__name__ + "_" + str(adv_train)]['means'] = []
        stats[attack_type.__name__ + "_" + str(adv_train)]['std'] = []
        ...
        for iter in range(stats_iter):
            ...
            # Modèle pour apprendre la distribution
            distribution = ReverseGaussian(n_classes=10, features=28 * 28, device='cuda').to('cuda')
            ...
            for e in range(10000):
                ...
                xchap = distribution(y)
                xchap = xchap.view(batch_size, 1, 28, 28)
                output = model(xchap)

                p = torch.exp(output)

                loss = F.nll_loss(output, y.view(batch_size, ))
            ...
            means = distribution.means.view(10,28, 28).detach()
            std = distribution.std.view(10,28*28, 28*28).detach()

            stats[attack_type.__name__ + "_" + str(adv_train)]['acc_generator'].append(acc.detach().cpu().numpy())
            stats[attack_type.__name__ + "_" + str(adv_train)]['means'].append(means.detach().cpu().numpy())
            stats[attack_type.__name__ + "_" + str(adv_train)]['std'].append(std.detach().cpu().numpy())

import pickle as pk

pk.dump(stats, open('tmp/stats_l1l2_fine_wholegenerator_gaussian.dat', 'wb'), protocol=pk.HIGHEST_PROTOCOL)
